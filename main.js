...
galleryEl.addEventListener("click", function (e) {
    ...
    let { paddingLeft: itemPaddLeft, paddingRight: itemPaddRight } =
    ...
    const zoom = Math.min(
        ...
        item.querySelector("img, video").naturalWidth /
            item.querySelector("img, video").offsetWidth
    );

    if (zoomedItem) {
        galleryEl.style.transitionProperty =
            "--left, --top, --zoom, transform-origin";
    } else {
    ...
    galleryEl.style.setProperty(
        "--left",
        `${galleryEl.offsetWidth / 2 - picCX}px`
    );
    galleryEl.style.setProperty(
        "--top",
        `${-item.offsetTop + scrollTop + (window.innerHeight - picH) / 2}px`
    );
